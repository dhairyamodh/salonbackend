    ...
    try {
        ...
        if (data == 'all') {
            await Promise.all(Object.values(global.salons).map(async (key) => {
                ...
                const branchData = await Promise.all(branch.map(async (values, index) => {
                    if (values.salonId) {
...
const create = async (data) => {
    try {
        ...
        if (await global.salons[data.salonId].Branch.create({ ...data, branchCode: getBranchCode(salon.name, salon.lastBranchCode || 0) })) {
            await Salon.findByIdAndUpdate(data.salonId, { $inc: { lastBranchCode: 1 } });
        }
...
module.exports = {
    create, getBranchBySalonId, update, remove
}