...
const create = async (data, files) => {
    try {
        ...
        const checkCategory = isSuperadminRole(data.role) ? await Category.find({ categoryName: data.categoryName }) : isSalonAdminRole(data.role) ? await global.salons[data.salonId].SalonCategory.find({ categoryName: data.categoryName }) : await global.salons[data.salonId].BranchCategory.find({ categoryName: data.categoryName });
        ...
        isSuperadminRole(data.role) ? await Category.create(data) : isSalonAdminRole(data.role) ? await global.salons[data.salonId].SalonCategory.create(data) : await global.salons[data.salonId].BranchCategory.create(data)
...
const update = async (data, files) => {
    try {
        ...
        isSuperadminRole(data.role) ? await Category.findByIdAndUpdate(data.id, data) : isSalonAdminRole(data.role) ? await global.salons[data.salonId].SalonCategory.findByIdAndUpdate(data.id, data) : await global.salons[data.salonId].BranchCategory.findByIdAndUpdate(data.id, data)
...
const remove = async (data) => {
    try {
        if (isSuperadminRole(data.role)) {
            ...
            await global.salons[data.salonId].SalonCategory.findByIdAndDelete(data.id)
        } else {
            await global.salons[data.salonId].BranchCategory.findByIdAndDelete(data.id);
        }

        if (!isSuperadminRole(data.role)) {
            !isSalonAdminRole(data.role) ? await global.salons[data.salonId].BranchService.updateMany({ categoryId: data.id }, { $set: { categoryId: undefined } }) : await global.salons[data.salonId].SalonService.updateMany({ categoryId: data.id }, { $set: { categoryId: undefined } });
        }
...
const importall = async (data) => {
    try {
        data.branchId ? await global.salons[data.salonId].BranchCategory.insertMany(data.data) : await global.salons[data.salonId].SalonCategory.insertMany(data.data)
