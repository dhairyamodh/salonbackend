...
const all = async (salonId, branchId, status) => {
    try {
        const data = { salonId: ObjectId(salonId), ...(branchId != undefined && { branchId: ObjectId(branchId) }), }
        let usergroups = [];
        if (salonId) {
        ...
        const userdata = await Promise.all(usergroups.map(async (item) => {
            let serviceLabel = ''
            if (item.services) {
...
const create = async (data) => {
    try {
        ...
        if (data.services === []) {
            return ({ status: httpStatus.INTERNAL_SERVER_ERROR, message: "Services is required" })
...
const update = async (data) => {
    try {
        await global.salons[data.salonId].SalonUserGroup.findByIdAndUpdate(data._id, data)
...
const remove = async (data) => {
    try {
        await global.salons[data.salonId].SalonUserGroup.findByIdAndDelete(data._id)
        await global.salons[data.salonId].SalonUserGroup.findOneAndDelete({ restaurantUserId: data._id })
