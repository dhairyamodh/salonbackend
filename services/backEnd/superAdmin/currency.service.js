...
const all = async (data, status) => {
    try {
        ...
        const currencyNameWithCybol = await Promise.all(result.map((curr, currIndex) => {
            return { ...curr._doc, currencyNameWithCybol: `${curr._doc.currencyName} (${curr._doc.currencySymbol})` }
        }))
...
const update = async (data) => {
    try {
        ...
        return ({ status: httpStatus.OK, message: 'Table type Updated Successfully' })
    } catch (error) {
        return ({ status: httpStatus.INTERNAL_SERVER_ERROR, message: error })
    }
}
...
const remove = async (data) => {
    try {
        ...
        return ({ status: httpStatus.OK, message: 'Table type Deleted Successfully' })
    } catch (error) {
        return ({ status: httpStatus.INTERNAL_SERVER_ERROR, message: error })
    }
}
